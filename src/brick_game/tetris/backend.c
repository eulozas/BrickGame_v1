...
void restartGameStruct(GameStruct_t *game) { initGameStruct(game); }

void clearField(GameStruct_t *game) {
  for (int i = 0; i < FIELD_HEIGHT; i++)
    for (int j = 0; j < FIELD_WIDTH; j++) game->field[i][j] = 0;
}
...
void copyGameInfo(GameStruct_t *game, GameInfo_t *info) {
  ...
  for (int i = 0; i < FIELD_HEIGHT; i++)
    for (int j = 0; j < FIELD_WIDTH; j++) info->field[i][j] = game->field[i][j];

  for (int i = 0; i < 4; i++)
    for (int j = 0; j < 4; j++) info->next[i][j] = game->next[i][j];
