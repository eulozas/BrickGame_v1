  ...
  for (int y = 0; y < 4 && exit_code != 0; y++) {
    for (int x = 0; x < 4 && exit_code != 0; x++) {
      if (game->currentPiece[y][x]) {
        ...
        if (fy >= FIELD_HEIGHT || (fy >= 0 && game->field[fy][fx] == 1))
          exit_code = 0;
      }
    }
  }
  ...
  for (int y = 0; y < 4 && exit_code != 0; y++) {
    for (int x = 0; x < 4 && exit_code != 0; x++) {
      if (game->currentPiece[y][x]) {
        int fx = game->currentX + x + 1;
        int fy = game->currentY + y;
        if (fx >= FIELD_WIDTH || (fy >= 0 && game->field[fy][fx] == 1))
          exit_code = 0;
      }
    }
  }
...
int canRotate(const GameStruct_t *game, const int rotated[4][4], int offsetX,
              int offsetY) {
  int exit_code = 1;
  for (int y = 0; y < 4 && exit_code != 0; y++) {
    for (int x = 0; x < 4 && exit_code != 0; x++) {
...
void rotatePiece(GameStruct_t *game) {
  ...
  for (unsigned int i = 0; i < sizeof(kicks) / sizeof(kicks[0]) && !was_rotated;
       i++) {
    int dx = kicks[i][0];
    int dy = kicks[i][1];
