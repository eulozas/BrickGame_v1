...
void block_input_ncurses() { nodelay(stdscr, FALSE); }

void noblock_input_ncurses() { nodelay(stdscr, TRUE); }

void delay_ncurses(int ms) { napms(ms); }
...
void printOverlay(const char *game_name) {
  clear();
  mvprintw(6, (FIELD_WIDTH * 2 + 2 - (int)strlen(game_name)) / 2, "%s",
           game_name);
  mvprintw(9, (FIELD_WIDTH * 2 + 2 - (int)strlen("Press S to start")) / 2,
           "Press S to start");
...
void printNextPiece(GameInfo_t *info) {
  ...
  for (int y = 0; y < 4; y++) {
    for (int x = 0; x < 4; x++) {
      mvprintw(start_y + y, start_x + x * 2, info->next[y][x] ? "<>" : "  ");
    }
  }
}

void printField(GameInfo_t *info) {
  for (int x = 0; x < FIELD_WIDTH * 2 + 2; x++) mvprintw(0, x, "#");

  for (int y = 0; y < FIELD_HEIGHT; y++) {
    mvprintw(y + 1, 0, "#");
    for (int x = 0; x < FIELD_WIDTH; x++) {
      mvprintw(y + 1, x * 2 + 1, info->field[y][x] ? "<>" : "  ");
    }
...
void drawGameInfo(GameInfo_t *info) {
  ...
  if (info->level == 0) {
    // Баннер START
    mvprintw(6, (FIELD_WIDTH * 2 + 2 - (int)strlen("THE GAME")) / 2,
             "THE GAME");
    mvprintw(7, (FIELD_WIDTH * 2 + 2 - (int)strlen("is ready to start")) / 2,
             "is ready to start");
    mvprintw(9, (FIELD_WIDTH * 2 + 2 - (int)strlen("Press S to start")) / 2,
             "Press S to start");
    mvprintw(10, (FIELD_WIDTH * 2 + 2 - (int)strlen("Press Q to exit")) / 2,
    ...
    mvprintw(7, (FIELD_WIDTH * 2 + 2 - (int)strlen("Press P to continue")) / 2,
             "Press P to continue");
    ...
    mvprintw(5, (FIELD_WIDTH * 2 + 2 - (int)strlen("GAME OVER")) / 2,
             "GAME OVER");
    mvprintw(7,
             (FIELD_WIDTH * 2 + 2 - (int)strlen("Press S to play again")) / 2,
    ...
    mvprintw(5, (FIELD_WIDTH * 2 + 2 - (int)strlen("high score file")) / 2,
             "high score file");
    mvprintw(6, (FIELD_WIDTH * 2 + 2 - (int)strlen("not found")) / 2,
             "not found");
    mvprintw(8, (FIELD_WIDTH * 2 + 2 - (int)strlen("Press S")) / 2, "Press S");
    mvprintw(9, (FIELD_WIDTH * 2 + 2 - (int)strlen("to start anyway")) / 2,
             "to start anyway");
    mvprintw(10, (FIELD_WIDTH * 2 + 2 - (int)strlen("Press Q to exit")) / 2,
             "Press Q to exit");
  } else {
